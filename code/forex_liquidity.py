...
# print(access_token)
access_token = "e7W2AjM31yiRIF10UrCPn4ND5T7Gk6nn"
...
def place_future_orders(ws,ticks):
    print('\nStarting next Tick')
    trades = kite.trades()
    for trade in trades[0:1]:
        time_diff = (datetime.datetime.now()-trade['fill_timestamp']).seconds
        if time_diff < 30*60:
            ws.stop()
    ...
    if current_rate < 72.6 or current_rate > 73.2:
        print("Yahoo Misbehaved: ",current_rate)
        ws.stop()
        return 
    ...
    for e in ticks:
        ...
        spot_price_sell = (
            current_rate * ( 1 + discount_rate_sell * (instrument["exp_date"] - current_date).days / 365 ) + 0.05
        )
        spot_price_buy = ( 
            current_rate * ( 1 + discount_rate_buy * (instrument["exp_date"] - current_date).days / 365 ) - 0.05
        )
        ...
        # debug depth
        print('\nMarket Sell Depth: ',e["depth"]["sell"])
        print('Market Buy Depth: ',e["depth"]["buy"])
        print('Spot_Prices: ',spot_price_buy,spot_price_sell)
